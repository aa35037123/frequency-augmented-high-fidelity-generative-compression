...
def load_model(save_path, logger, device, model_type=None, model_mode=None, current_args_d=None, prediction=True, 
    strict=False, silent=False, fix_pretrain=False):
    ...
    if fix_pretrain is True:
        logger.info('MODEL TYPE: {}'.format(model_type))
        logger.info('MODEL MODE: {}'.format(model_mode))
        logger.info(model)
        logger.info('Trainable parameters:')
        for name, param in model.named_parameters():
            if ('sigma' not in name) and ('FCM' not in name):
                param.requires_grad = False
            logger.info('{} - {}, require_grad={}'.format(name, param.shape, param.requires_grad))
...
def log_summaries(writer, storage, step, use_discriminator=False):
    ...
    weighted_compression_scalars = ['weighted_compression_loss',
                                    ...
                                    'weighted_distortion',
                                    'weighted_perceptual',
                                    'weighted_focal_frequancy_loss',
                                    'weighted_dynamic_spectrum_loss',
                                    'rate_penalty']
    ...
    compression_loss_breakdown = dict(total_comp=storage['weighted_compression_loss'][-1],
                                      ...
                                      weighted_focal_frequancy_loss=storage['weighted_focal_frequancy_loss'][-1],
                                      weighted_dynamic_spectrum_loss=storage['weighted_dynamic_spectrum_loss'][-1])
...
def log(model, storage, epoch, idx, mean_epoch_loss, current_loss, best_loss, start_time, epoch_start_time, 
    ...
    report_f("Weighted R-D: {:.3f} | Weighted Rate: {:.3f} | Weighted Distortion: {:.3f} | Weighted Perceptual: {:.3f} | Weighted FFL: {:3f} | Weighted DSL: {:3f} |"
             "Distortion: {:.3f} | Rate Penalty: {:.3f}".format(storage['weighted_R_D'][-1],
             storage['weighted_rate'][-1], storage['weighted_distortion'][-1], storage['weighted_perceptual'][-1], storage['weighted_focal_frequancy_loss'][-1], storage['weighted_dynamic_spectrum_loss'][-1],
