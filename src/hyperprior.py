...
class CodingModel(nn.Module):
    ...
    def _quantize(self, x, mode='noise', means=None):
        """
        # hard quantization: transform continuous into discrete
    ...
    def _estimate_entropy(self, likelihood, spatial_shape): # input likelihood is not log

        EPS = 1e-9  # error value
        quotient = -np.log(2.)
        ...
        bpp = n_bits / n_pixels # bit per pixel 

        return n_bits, bpp

    def _estimate_entropy_log(self, log_likelihood, spatial_shape): # input likelihood is not log
    ...
    def latent_likelihood(self, x, mean, scale):
        ...
        x = x - mean
        x = torch.abs(x)
        cdf_upper = self.standardized_CDF((0.5 - x) / scale) # x >= 0.5-x
        cdf_lower = self.standardized_CDF(-(0.5 + x) / scale) # x < -0.5 - x
...
class Hyperprior(CodingModel):
    ...
    def __init__(self, bottleneck_capacity=220, hyperlatent_filters=LARGE_HYPERLATENT_FILTERS, 
        ...
        super(Hyperprior, self).__init__(n_channels=bottleneck_capacity) # means dimension of latent 
    ...
    # it means decoder, take z as input, and output mean and scale
    ...
    def forward(self, latents, spatial_shape, **kwargs):
        ...
        noisy_hyperlatents = self._quantize(hyperlatents, mode='noise') # use adding noise to quantize
        ...
        # hyper is used to get mean and std to make decode perform well
        latent_means = self.synthesis_mu(hyperlatents_decoded)
        latent_scales = self.synthesis_std(hyperlatents_decoded)
