...
class Model(nn.Module):
    ...
    def __init__(self, args, logger, storage_train=defaultdict(list), storage_test=defaultdict(list), model_mode=ModelModes.TRAINING, 
        ...
        # check that whether model have these attributes
        ...
        # no need to use discriminator when testing
        ...
        if self.use_discriminator is True:
            ...
            self.gan_loss = partial(losses.gan_loss, args.gan_loss_type) # partial(func, args of func in front)
        else:
    ...
    def compression_forward(self, x):
        ...
        if self.model_mode == ModelModes.EVALUATION and (self.training is False):
            ...
            x = utils.pad_factor(x, x.size()[2:], factor) # padding x (input)

        # Encoder forward pass
        y, A1_gaussian, A2_gaussian = self.Encoder(x)
        ...
        reconstruction, C1_gaussian, C2_gaussian = self.Generator(latents_quantized)
        ...
        return intermediates, hyperinfo, A1_gaussian, A2_gaussian, C1_gaussian, C2_gaussian
    ...
    def discriminator_forward(self, intermediates, train_generator):
        ...
        latents = intermediates.latents_quantized.detach() # detach() means cannot train
        latents = torch.repeat_interleave(latents, 2, dim=0) # torch.repeat_interleave(input, repeat_time, along what dimension)

        D_out, D_out_logits = self.Discriminator(D_in, latents)
        D_out = torch.squeeze(D_out)
        D_out_logits = torch.squeeze(D_out_logits) # squeezee 
        

        D_real, D_gen = torch.chunk(D_out, 2, dim=0) # chunk can divise tensor into multiple part
        D_real_logits, D_gen_logits = torch.chunk(D_out_logits, 2, dim=0)
        # print((D_real, D_gen, D_real_logits, D_gen_logits))

        return Disc_out(D_real, D_gen, D_real_logits, D_gen_logits)

    def distortion_loss(self, x_gen, x_real): # MSE in dp
        # loss in [0,255] space but normalized by 255 to not be too big
        # - Delegate scaling to weighting
        sq_err = self.squared_difference(x_gen*255., x_real*255.) / 255.
        # print('sq_err', sq_err.shape, print(torch.mean(sq_err)))
        return torch.mean(sq_err)

    def perceptual_loss_wrapper(self, x_gen, x_real, normalize=True): # kp in d
        ...
        # print(torch.mean(LPIPS_loss))
        return torch.mean(LPIPS_loss)
    
    def focal_frequancy_loss(self, gen, real):
        ffl = FFL(loss_weight=1.0, alpha=1.0)
        FFL_loss = ffl(gen, real)

        return FFL_loss

    def compression_loss(self, intermediates, hyperinfo, A1, A2, C1, C2):
        ...
        focal_frequancy_loss_pixel = self.focal_frequancy_loss(x_gen, x_real)
        DSL1 = self.focal_frequancy_loss(C1, A1) / 60
        DSL2 = self.focal_frequancy_loss(C2, A2) / 120
        # print(DSL1, DSL2)
        focal_frequancy_loss_feature = DSL1 + DSL2

        weighted_distortion = self.args.k_M * distortion_loss
        weighted_perceptual = self.args.k_P * perceptual_loss
        weighted_ffl = self.args.k_ffl * focal_frequancy_loss_pixel
        weighted_dsl = self.args.k_dsl * focal_frequancy_loss_feature
        weighted_frequancy = weighted_ffl + weighted_dsl
        ...
        weighted_compression_loss = weighted_R_D_loss + weighted_perceptual + weighted_frequancy
        ...
        if (self.step_counter % self.log_interval == 1):
            ...
            self.store_loss('weighted_focal_frequancy_loss', weighted_ffl.item())
            self.store_loss('weighted_dynamic_spectrum_loss', weighted_dsl.item())
    ...
    # # compress return the latent after compress
    def compress(self, x, silent=False): # for testing
        ...
        * Encode latents via entropy model derived from (mean, scale) hyperprior output.
        ...
        # compression will pass through hyperprior, get the means and scaler 
        # compress_forward return the reconstruction img
    ...
    # with compression
    def decompress(self, compression_output): # for testing 
    ...
    def forward(self, x, train_generator=False, return_intermediates=False, writeout=True):
        ...
        losses = dict()
        if train_generator is True: # train_generator enable grad to generator
            # Define a 'step' as one cycle of G-D training
            self.step_counter += 1

        intermediates, hyperinfo, A1, A2, C1, C2 = self.compression_forward(x)
        ...
        compression_model_loss = self.compression_loss(intermediates, hyperinfo, A1, A2, C1, C2)
        ...
        if self.use_discriminator is True:
            ...
            weighted_G_loss = self.args.beta * G_loss # generator loss
