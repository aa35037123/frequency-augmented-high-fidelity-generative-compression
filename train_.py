...
def test(args, model, epoch, idx, data, test_data, test_bpp, device, epoch_test_loss, storage, best_test_loss, 
    ...
    with torch.no_grad():
        ...
        activate_folder = 'activate'
        losses, intermediates, A1, A2, C1, C2 = model(data, return_intermediates=True, writeout=False)
        ...
        losses, intermediates, A1, A2, C1, C2 = model(test_data, return_intermediates=True, writeout=True)
        ...
        for idx, (data, bpp) in enumerate(tqdm(train_loader, desc='Train'), 0):
            ...
            try:
                if model.use_discriminator is True:
                    ...
                    losses, A1, A2, C1, C2 = model(data, train_generator=train_generator)
                    compression_loss = losses['compression']
                    ...
                    losses, A1, A2, C1, C2 = model(data, train_generator=True)
                    compression_loss = losses['compression']
                    optimize_compression_loss(compression_loss, amortization_opt, hyperlatent_likelihood_opt)
...
if __name__ == '__main__':
    ...
    general.add_argument("-mm", "--model_mode",default='training', choices=(ModelTypes.TRAINING, ModelTypes.VALIDATION, ModelTypes.EVALUATION))
