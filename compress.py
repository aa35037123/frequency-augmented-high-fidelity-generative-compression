    ...
    with torch.no_grad():
        ...
        for idx, (data, bpp, filenames) in enumerate(tqdm(eval_loader), 0):
            ...
            if args.reconstruct is True:
                ...
                reconstruction, q_bpp = model(data, writeout=False) # compress return the reconstruction img
            else:
                # Perform entropy coding
                compressed_output = model.compress(data) # compress return the latent after compress
